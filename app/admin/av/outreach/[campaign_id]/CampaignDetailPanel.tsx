          ...
          <div className="flex items-center gap-2">
            {campaign.status === 'paused' || campaign.status === 'draft' ? (
              ...
              <button
                ...
                disabled={busy}
                className="px-3 py-1.5 text-sm rounded-md bg-[var(--gold-bright)] text-ink hover:bg-[var(--gold-bright)] disabled:opacity-50"
              >
                Pause
              </button>
            ) : null}
            ...
            {pending.map((p) => (
              <li key={p.id} className="rounded-lg border border-border bg-[var(--surface)] p-3">
                ...
                <div className="text-xs text-muted mb-2">
                  ...
                  {p.ai_grounded_on_audit ? <span className="ml-2 text-[var(--gold-bright)]">· audit-grounded</span> : null}
                </div>

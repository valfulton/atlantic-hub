...
export function IntakeDraftEditor({ auditId }: { auditId: string }) {
  ...
  if (!open) {
    return (
      <button
        ...
        className="text-xs px-3 py-1.5 rounded-md border border-border bg-black/20 text-ink hover:border-amber-400/40 inline-flex items-center gap-1.5"
  ...
  return (
    <div className="rounded-2xl border border-amber-400/25 bg-amber-400/[0.04] p-4 mb-4">
            ...
            {priority.map((f) => (
              <li key={f.key} className="rounded-md border border-white/5 bg-black/20 p-2.5">
                ...
                <textarea
                  ...
                  className="w-full bg-black/30 border border-white/10 rounded px-2 py-1.5 text-[12px] text-white/90 placeholder-white/30 focus:outline-none focus:border-amber-400/40"

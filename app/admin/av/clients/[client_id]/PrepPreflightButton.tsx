      ...
      {report && (
        <div className="mt-3 grid gap-2 text-xs">
          <div className="flex items-center gap-4 text-[11px] text-muted flex-wrap">
            ...
            <span><span className="text-amber-300">{willSkip}</span> would skip</span>
            {report.url ? (
            ...
            <span>brief: <span className={report.brief.enoughForLlm ? 'text-emerald-300' : 'text-amber-300'}>{report.brief.filledCount} field{report.brief.filledCount === 1 ? '' : 's'} filled</span></span>
          </div>
          <ul className="grid gap-1 mt-1">
            {Object.entries(report.steps).map(([key, s]) => (
              <li key={key} className="flex items-start gap-2">
                <span aria-hidden className={`inline-block w-1.5 h-1.5 rounded-full mt-1.5 shrink-0 ${s.ok ? 'bg-emerald-400' : 'bg-amber-400'}`} />
                <span className="text-ink/90 shrink-0">{STEP_LABEL[key] ?? key}</span>
                <span className={s.ok ? 'text-emerald-300' : 'text-amber-200/85'}>

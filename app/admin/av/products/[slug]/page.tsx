/**
 * /admin/av/products/[slug]  (val 2026-06-11)
 *
 * OPERATOR-ONLY product detail page. Moved from public /products/[slug]
 * per val 2026-06-11. Behind /admin/* JWT wall.
 *
 * Renders from the static product registry at lib/av/products.ts.
 * Static generation: every product in the registry pre-renders at build
 * time via generateStaticParams + force-static. Adding a new product
 * means adding to PRODUCTS — no manual route work needed.
 */
...
export default function ProductDetailPage({ params }: { params: { slug: string } }) {
  ...
  const verticalSlug =
    product.verticalPackId === 'collections' ? '/admin/av/products/collections-intelligence'
    : `/admin/av/products/${product.verticalPackId.replace(/_/g, '-')}-intelligence`;
  const statusBadge =
  ...
  return (
    <main style={{ background: '#FFFDF5', color: '#0A0A0A', minHeight: '100vh', padding: '3rem 1.5rem 4rem' }}>
      <div style={{ maxWidth: 860, margin: '0 auto' }}>
        ...
        {/* Pricing (internal planning view) */}
        <Section title="Pricing (internal — not public)">

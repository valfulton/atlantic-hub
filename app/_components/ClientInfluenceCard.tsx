...
export default async function ClientInfluenceCard({
  ...
  const wrapper =
    variant === 'inline'
      ? 'rounded-xl border border-[color-mix(in_srgb,var(--gold-bright)_15%,transparent)] bg-[var(--gold-bright)]/[0.03] p-3'
      : 'rounded-2xl border border-border bg-surface p-4';

  if (!seed) {
    return (
      <div className={`${wrapper} ${className}`}>
        <div className="text-[11px] uppercase tracking-[0.12em] text-[color-mix(in_srgb,var(--gold-bright)_75%,transparent)] mb-1">
          {clientName} — influence
        </div>
        <div className="text-[12px] text-white/55 italic">
          No brief on file yet. <Link href={`/admin/av/brief?clientId=${clientId}`} className="text-[color-mix(in_srgb,var(--gold-bright)_85%,transparent)] hover:underline">Add their brief</Link> so the content engine grounds in their voice.
        </div>
      </div>
    );
  }
  ...
  return (
    <div className={`${wrapper} ${className}`}>
      <div className="flex items-baseline justify-between gap-3 mb-2.5 flex-wrap">
        <div>
          <div className="text-[11px] uppercase tracking-[0.12em] text-[color-mix(in_srgb,var(--gold-bright)_85%,transparent)]">
            {clientName} — influence
          </div>
        ...
        <Link
          ...
          className="text-[10px] uppercase tracking-wider text-white/50 hover:text-[var(--gold-bright)] transition shrink-0"
        >
          Edit brief →
        </Link>
      </div>
      ...
      {(seed.brandColors || brandAesthetic || brandTypography || logoUrl) && (
        <div className="mt-3 pt-3 border-t border-white/5">
          <div className="text-[9.5px] uppercase tracking-[0.16em] text-[color-mix(in_srgb,var(--gold-bright)_75%,transparent)] mb-2">
            Visual identity
          </div>

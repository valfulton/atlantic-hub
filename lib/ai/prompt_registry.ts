...
const WEBSITE_AUDIT_DEFAULT = [
  ...
  `{2-3 sentences. If prices are visible, NAME the prices and tiers you see (e.g. "Single 12-pack $50, Subscription $39/mo, Wholesale on request"). If hidden, say so AND say whether that's right for the industry. This is the operator's #1 question on calls — give them ammo.}`,
  ``,
...
const INTAKE_WEB_FILLER_DEFAULT = [
  ...
  `6. NEVER include AI/API call costs or platform-side prices. Client-side product/service prices (what they charge their customers, visible on the site) ARE useful and SHOULD be captured — see rule 8.`,
  ...
  `8. PRICING + DEAL ECONOMICS (#515, val 2026-06-08). When the page shows prices, pricing tiers, "starting at $X", subscription rates, or package costs, capture them:`,
  `   - brand_pricing: short positioning ("luxury / premium / mid-market / value"). Decide from price tier + visual register.`,
  `   - avg_deal_value: numeric USD estimate of a typical sale. Derive from displayed prices when possible — e.g. "Starting at $1,995/mo" → 1995; "12-pack $50, 24-pack $90" → 70. If only a range is visible (e.g. "$1k-$5k"), use the midpoint.`,
  `   - additional_info: if the page has substantive pricing detail worth preserving (specific tier names, what each tier includes, contract terms, money-back guarantees), include it here as compact prose. This is where rich pricing intel that doesn't fit elsewhere lives so the rep / brief has it for the call.`,
  `   Goal: a sales rep should be able to read the intake and know what the client charges and how a customer buys, without re-visiting the site.`,
